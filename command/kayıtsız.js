...
let set = require("../ayarlar/setting")


exports.run = async(client, message , args) => {
    if (!message.member.roles.cache.has(set.registerYetkili) && !message.member.permissions.has("Administrator")) return message.reply({ content : `${emoji.red} **Bu komutu kullanmak için yetkin bulunmamaktadır!**`}).then((e) => setTimeout(() => { if(e.deletable){e.delete()}; }, 10000));
...
await member.roles.cache.has(set.boosterRole) ? member.roles.set([set.boosterRole,set.kayıtsızRol]) : member.roles.set([set.kayıtsızRol])
